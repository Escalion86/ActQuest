...
const TelegramLogin = ({ availableLocations, location, onLocationChange, onAuth, isClient }) => {
  ...
  return (
    <div className="mx-auto mt-12 max-w-4xl rounded-3xl bg-white p-8 shadow-lg dark:border dark:border-slate-800 dark:bg-slate-900 dark:shadow-slate-950/40">
      <h2 className="text-2xl font-bold text-primary dark:text-white">Войти через Telegram</h2>
      ...
      <div className="mt-6 flex flex-col gap-6">
        <label className="flex flex-col gap-2 text-sm font-medium text-gray-700 dark:text-slate-200">
          Регион
          <select
            className="rounded-xl border border-gray-200 px-4 py-3 text-base shadow-sm focus:border-blue-400 focus:outline-none focus:ring dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
            value={location}
        ...
        <div className="flex flex-col items-start gap-4">
          {botName && isClient ? (
            ...
            <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-4 py-6 text-gray-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
              Укажите название бота для региона в переменной окружения{' '}
              <code className="rounded bg-gray-200 px-1 dark:bg-slate-700 dark:text-slate-100">NEXT_PUBLIC_TELEGRAM_{location.toUpperCase()}_BOT_NAME</code>
            </div>
          )}
