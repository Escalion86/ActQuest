...
**TeamJoinRequests** (`teamJoinRequestsSchema.js`)
- Заявка пользователя: `teamId`, `userId`, `status`
  (`pending|accepted|rejected`)
- Решение капитана: `processedByUserId`, `processedAt`
- При новой заявке капитану создаётся уведомление и отправляется Web Push со
  ссылкой на управление заявками команды. Количество ожидающих заявок
  показывается у пункта «Мои команды» и на карточке капитанской команды.
...
- Персональное место команды (`userTeamPlace`) не передаётся обычному игроку и
  не отображается в «Обзоре» или карточках игр, пока `hideResult === true`.
  Сама сыгранная игра при этом остаётся в истории с пометкой «Результаты скрыты».
...
3. **Отображение в Обзоре (Overview):**
   ...
   - Для опубликованного результата игра должна иметь `userTeamPlace > 0`.
   - При `hideResult === true` сыгранная игра остаётся в истории участника, но
     `userTeamPlace` скрывается и интерфейс показывает «Результаты скрыты».
...
- Snapshot рейтинга имеет `version: 2`. Очки отдельной игры нормализуются по
  формуле `100 × (число соперников + 1 − место) / число соперников`: первое
  место даёт `100`, последнее — `0`. Игры с единственным участником в рейтинг
  не входят.
- `finalScore` и `ratingPoints` содержат среднее число очков 0–100; большее
  значение означает более высокое место. Для допуска нужно минимум три игры.
- Пропуски не меняют `finalScore`; `attendance` и `missedGames` остаются
  информационными метриками.
- Основной рейтинг кабинета хранится в `ratingsByLocation[location]` и считается
  за всё время только по играм выбранного в профиле города. `rating` хранит
  общий all-time snapshot для обратной совместимости и служебных сценариев.
- Рейтинги отдельных сезонов хранятся в `ratingsBySeason[seasonId]`. Страницы
  рейтинга позволяют переключаться между городским зачётом за всё время и
  сезонами, в которых есть закрытые рейтинговые игры этого города.
...
- При равном `finalScore` используются тай-брейки: больше игр, больше побед,
  лучший последний результат. Полностью одинаковые показатели получают одно
  место (tie rank).
...
- Удаляет: Teams, TeamsUsers, GamesTeams, TeamJoinRequests
- UI: кнопка в `TeamEditModal`, доступна капитану команды
