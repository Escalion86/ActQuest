...
- Snapshot рейтинга имеет `version: 2`. Очки отдельной игры нормализуются по
  ...
  не входят.
- `finalScore` и `ratingPoints` содержат число очков 0–100; большее значение
  означает более высокое место. Для допуска нужно минимум три сыгранные игры.
- В общем зачёте за всё время `finalScore` равен среднему только по сыгранным
  играм, а `attendance` и `missedGames` остаются информационными метриками.
- В сезонном зачёте `finalScore` равен сумме очков за сыгранные этапы, делённой
  на число всех закрытых рейтинговых этапов сезона. Пропущенный этап тем самым
  даёт 0 очков и не может улучшить сезонную позицию.
