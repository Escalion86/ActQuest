...
- [x] Добавлены отзывы участников после игры (2026-07-27): персональная оценка
  1–10, теги и текст на странице результата; сервер проверяет участие по
  `result.teamsUsers`, один игрок может оставить один редактируемый отзыв на
  игру. В кабинете показывается статус отзыва, а в админке доступны фильтры,
  средняя оценка, согласие на публикацию и модерация. Добавлен
  `npm run test:game-reviews`.
