...
- [~] Перевести описание задания на rich-text редактор (TipTap): форматирование + inline медиа (`taskRich`, `taskMedia`) с web-readonly рендером; в кабинете внедрён notion-like UI (bubble menu + slash-команды).
