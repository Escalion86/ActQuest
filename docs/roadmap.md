...
- [~] Поддержка полевых агентов: добавлена роль `agent`, пул агентов в настройках игры (`Games.agents`), общие настройки агентских уведомлений (`Games.agentNotifications`), привязка агентов к заданиям (`tasks[].agentUserIds`) и story-локациям (`storyNodes[].agentUserIds`) с каскадным удалением из назначений при удалении агента из игры, агентский кабинет `/cabinet/agent`, безопасный статус команд `/api/cabinet/agent/game-status` и idempotent push-уведомления через `server/agentNotifications.js`. Следующий шаг: ручная проверка сценариев push-подписок на стенде.
