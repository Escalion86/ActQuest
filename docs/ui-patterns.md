...
- Для приквела:
  - описание можно показывать всем;
  - форму ввода показывать только капитану зарегистрированной команды;
  - после `dateStartFact` форма ввода должна стать read-only;
  - правильные приквельные коды не должны уходить в клиентские payload'ы.
...
Администратор, разработчик, создатель или назначенный модератор может открыть
управляемую игру через действие «Тестовый прогон» на карточке игры.

- `POST /api/cabinet/admin/game-test-runs` создаёт либо продолжает прогон;
- `POST` с `action: 'reset'` пересоздаёт прогон на свежем снимке игры;
- `DELETE /api/cabinet/admin/game-test-runs?runId=...` завершает прогон;
- игроковый маршрут: `/game/[id]/test/[runId]`;
- прогресс хранится отдельно в `GameTestRuns`, а не в `GamesTeams`;
- внутри снимка статус игры принудительно равен `started`, исходная игра не
  меняет статус;
- тестовые действия не отправляют уведомления агентам и не участвуют в
  Game Control, результатах, рейтинге или статистике;
- classic/photo используют `/api/webapp/game-task` с `testRunId`, story — те же
  player story API с query-параметром `testRunId`;
- доступ к каждому чтению и изменению прогресса повторно проверяется по владельцу
  тестового прогона.

Тестовая сессия живёт семь дней, после чего удаляется TTL-индексом MongoDB.
