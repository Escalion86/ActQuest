...
Редактор открывается из карточки story-игры по адресу
`/cabinet/admin/story-editor?gameId=...`.

1. Нажмите **«Добавить локацию»** в верхней панели: откроется форма новой сцены.
   Задайте название и описание для игроков. Раздел **«Локации»** позволяет
   вернуться к любой созданной сцене; **«Предметы»** открывает каталог предметов.
2. В разделе **«Доступность, баллы и координаты»** обычного story-квеста
   выберите **«Доступна с начала игры»** для стартовой
   локации. У первой созданной сцены этот параметр уже включён. Добавьте код
   ответа или действие и при необходимости отметьте **«Завершает локацию»**.
3. Через **«Связи»** задайте условия открытия следующих локаций. В форме локации
   выберите, должны ли выполняться все условия, любое одно или заданное число.
   Через **«Концовки»** настройте завершение квеста.
4. Для режима расследования выберите стартовую локацию и лимит времени в
   **«Настройках»**. Задания создавайте через взаимодействия: вопросы, осмотры,
   анализ с условиями и результатами. Настройте финальное обвинение и концовки.
5. Нажмите **«Вернуться к сценарию»**, затем **«Сохранить сценарий»** на основном
   экране. Закрытие формы само по себе не сохраняет изменения на сервере.
   Исправьте сообщения проверки перед запуском игры.

Краткая инструкция **«Как настроить сценарий»** доступна над рабочим полем;
в пустом сценарии она раскрыта. Использованные в игре сценарии остаются
доступными только для просмотра — для изменений нужна копия игры.

Название и описание находятся в начале формы локации. Необязательные медиа
собраны в раскрывающемся разделе **«Обложка, аудио и видео»**. Добавление
локации или предмета сразу открывает форму, добавление концовки выбирает её
в редакторе. Идентификатор новой сущности создаётся до обновления состояния,
чтобы повторный вызов React updater не нарушал выбор созданного элемента.
