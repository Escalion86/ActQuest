...
const fetchGamesForCabinet = async ({
  ...
  // Построить query для поиска игр
  let query = null

  // Если есть ID пользователя, ищем игры через result.teamsUsers (где пользователь когда-то играл)
  if (hasUserId || hasTelegramId) {
    const userOrConditions = []
    if (hasUserId) {
      userOrConditions.push({ 'result.teamsUsers.userId': currentUserIdString })
    }
    if (hasTelegramId) {
    ...
    // Добавить ограничения location и hidden
    const normalizedLocation =
      typeof location === 'string' ? location.trim().toLowerCase() : null
    if (normalizedLocation) {
      query.location = normalizedLocation
    ...
    // Обычные пользователи видят только públicos (hidden !== true)
    const canLoadAllGames = userRole === 'admin' || userRole === 'dev'
    if (!canLoadAllGames) {
      query.hidden = { $ne: true }
    }
  } else {
    // Если нет ID пользователя, используем стандартный query
  ...
  // Если поиск был по result.teamsUsers, заполнить данные из snapshot'ов
  if (hasUserId || hasTelegramId) {
    gamesFiltered.forEach((game) => {
      ...
      const userTeamsFromSnapshot = resultTeamsUsers.filter((tu) => {
        ...
        const matchesTelegramId =
          hasTelegramId && Number(tu?.userTelegramId) === currentUserTelegramIdNumber
      ...
      // Установить место из snapshot'ов с fallback'ами
      if (places.length > 0) {
      ...
      // Заполнить teamsCountMap из result.teams
  ...
  if ((hasUserId || hasTelegramId) && query) {
    ...
    if (historyOrConditions.length > 0) {
      // Построить базовый query с location и hidden ограничениями
      const baseHistoryQuery = {}
      if (typeof query?.location === 'string') {
        baseHistoryQuery.location = query.location
      }
      if (query?.hidden !== undefined) {
        baseHistoryQuery.hidden = query.hidden
      }

      // Присоединить условие поиска по snapshot'ам
      baseHistoryQuery.$or = historyOrConditions

      const historicalGamesDocs = await GamesModel.find(baseHistoryQuery)
        .sort(resolveGamesSort(view))
        .select({
          _id: 1,
          name: 1,
  ...
  // Функция для обработки snapshot'ов и заполнения userTeamPlace и userParticipationTeams
  const processSnapshotData = (game) => {
    const gameId = toStringId(game?._id)
  ...
  if (gameIds.length) {
    ...
    if (gamesTeams.length > 0 && (hasUserId || hasTelegramId)) {
      ...
      gamesFiltered.forEach((game) => {
        ...
        if (places.length > 0) {
          ...
          // Используем последнее место (количество команд) или 1 как fallback
          const resultTeamsCount = Array.isArray(game?.result?.teams)
            ? game.result.teams.length
            : null
          userTeamPlaceByGameId[gameId] = Number.isFinite(resultTeamsCount)
          ...
          // Даже если snapshotTeamIds пусто (corrupted data), но пользователь есть в snapshot,
          // установить минимальное место, чтобы игра была видна
          userTeamPlaceByGameId[gameId] = 1
        }
    ...
    // ГАРАНТИРОВАТЬ, что все игры в gamesFiltered имеют userTeamPlace
    // Обработать игры через snapshot'ы, которые не были обработаны выше
    gamesFiltered.forEach((game) => {
      const gameId = toStringId(game?._id)
      if (!gameId || userTeamPlaceByGameId[gameId] !== undefined) {
        // Уже обработана или нет ID
        return
      }

      // Поиск пользователя в snapshot'ах
      const resultTeamsUsers = Array.isArray(game?.result?.teamsUsers)
        ? game.result.teamsUsers
        : []
      const userTeamsFromSnapshot = resultTeamsUsers.filter((tu) => {
      ...
      // Заполнить userParticipationTeamsByGameId
      userParticipationTeamsByGameId[gameId] = userTeamsFromSnapshot.map(
        (tu) => {
          const teamId = toStringId(tu?.teamId)
          const snapshotTeam = teamId ? resultTeamsById[teamId] : null
  ...
  // Обработать snapshot'ы для всех оставшихся игр (которые не были обработаны выше)
  // Это КРИТИЧНО: нужно вызвать для всех игр в gamesFiltered, чтобы гарантировать userTeamPlace
  if (Array.isArray(gamesFiltered) && gamesFiltered.length > 0) {
    gamesFiltered.forEach((game) => {
      processSnapshotData(game)
    })
  }
