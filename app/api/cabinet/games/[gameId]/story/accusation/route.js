...
import { submitInvestigationAccusation } from '@server/storyInvestigationEngine'
...
export async function POST(request, { params }) {
  try {
    ...
    const culpritId = normalizeStringId(payload?.culpritId)
    const motiveId = normalizeStringId(payload?.motiveId)
    const evidenceIds = Array.isArray(payload?.evidenceIds)
      ? payload.evidenceIds.slice(0, 20).map(normalizeStringId).filter(Boolean)
      : []
    if (!culpritId || !motiveId) {
      return NextResponse.json(
        { success: false, error: 'Не указан подозреваемый или мотив' },
        { status: 400 },
      )
    }
    ...
    console.error('Failed to submit story investigation accusation', error)
    return NextResponse.json(
      { success: false, error: 'Не удалось предъявить обвинение' },
      { status: 500 },
    )
  }
}
