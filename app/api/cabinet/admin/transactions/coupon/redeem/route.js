...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: redeemed }, { status: 201 })
  } catch (error) {
    console.error('Redeem coupon API error (app)', error)
    return NextResponse.json(
      { success: false, error: error?.message || 'Не удалось погасить купон' },
      { status: 400 },
    )
  }
}
