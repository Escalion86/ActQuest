...
const collectTelegramIdsForLocation = async ({ db, location }) => {
  ...
  const teamIds = Array.from(
    new Set(gameTeams.map((item) => (item?.teamId ? String(item.teamId) : null)).filter(Boolean))
  )
...
export default async function handler(req, res) {
  ...
  const targetLocation = normalizeLocation(req.body?.location) || 'all'

  if (!message) {
    return res.status(400).json({ success: false, error: 'Введите сообщение для рассылки' })
  }

  const supportedLocations = resolveSupportedLocations()
  if (targetLocation !== 'all' && !supportedLocations.includes(targetLocation)) {
    return res.status(400).json({ success: false, error: 'Некорректный город для рассылки' })
  }
  ...
  try {
    ...
    if (missingTokenLocations.length > 0) {
      const details = missingTokenLocations
        .map((locationKey) => {
          const envKey = TELEGRAM_TOKEN_ENV_BY_LOCATION[locationKey] || 'UNKNOWN_TOKEN'
    ...
    const uniqueSentTelegramIds = Array.from(
      new Set(sentRecipients.map((item) => Number(item.chatId)).filter((id) => Number.isFinite(id)))
    ).sort((a, b) => a - b)
