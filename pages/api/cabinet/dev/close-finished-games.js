...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const gameSource of finishedGames) {
      ...
      if (hasResultSnapshots(game)) {
        try {
          ...
          console.error('Failed to rebuild result during force close', buildError)
        }
      }
      ...
      const updateStatsInfo = await updateParticipantsClosedStats({ db, game: updatedGame })
      const updateRatingsInfo = await updateParticipantsRatings({ db, game: updatedGame })
    ...
    console.error('Failed to close finished games', error)
