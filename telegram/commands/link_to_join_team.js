...
const link_to_join_team = async ({ telegramId, message, props }) => {
  ...
  return {
    message: `Код для присоединения к команде: <code>${props?.teamId}</code>.\nКликните по коду, чтобы скопировать его, затем отправьте пользователю, которого хотите пригласить. Этот кот необходимо ввести в поле "Присоединиться к команде"`,
